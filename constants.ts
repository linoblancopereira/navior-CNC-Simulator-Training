...
Después de realizar un desbaste con G71, G72 o G73, la pieza queda con una sobremedida (definida por U y W en el ciclo de desbaste). El ciclo G70 se utiliza para realizar una sola pasada final siguiendo el contorno programado y eliminar esa sobremedida.
...
El ciclo G70 utiliza el mismo avance (F) y velocidad (S) programados dentro del contorno P-Q.
    `,
    defaultCode: `N10 G28 U0 W0
N20 T0101 (Desbaste)
N30 G96 S200 M03
...
El ciclo G75 se utiliza para mecanizar ranuras en el diámetro exterior o interior (eje X). Rompe la viruta retrocediendo la herramienta periódicamente.

**Sintaxis:**
1. \`G75 R(retorno)\`
2. \`G75 X(final) Z(final) P(inc. X) Q(inc. Z) F(avance)\`
...
* **R:** Retorno para romper viruta.
    `,
    defaultCode: `N10 G28 U0 W0
N20 T0202 (Herramienta Ranurado 3mm)
N30 G97 S1000 M03
N40 G00 X52 Z-20
(Inicio Ciclo G75)
N50 G75 R1
...
El G76 es el ciclo más completo para realizar roscas en múltiples pasadas.

**Sintaxis:**
1. \`G76 P(m)(r)(a) Q(min) R(acabado)\`
2. \`G76 X(final) Z(final) P(altura) Q(primera) F(paso)\`

* **P(mra):** m=repeticiones, r=salida angular, a=ángulo hta (60°).
* **X, Z:** Coordenadas finales de la rosca.
* **P(altura):** Altura del filete (micras).
* **F:** Paso de la rosca (mm).
    `,
    defaultCode: `N10 G28 U0 W0
N20 T0303 (Hta Roscado 60deg)
N30 G97 S800 M03
N40 G00 X30 Z5
(Ciclo G76 M24x2.0)
(P010060: 1 pasada acabado, salida 0, angulo 60)
...
Aunque G74 se usa para ranurado frontal, es común usarlo para taladrado profundo con rotura de viruta (Peck Drilling) en el eje Z.
...
N30 G97 S800 M03
N40 G00 X0 Z5 (Posicion en centro)
(Ciclo de Taladrado)
N50 G74 R1
...
Funciona de forma similar al G71, pero desbasta en dirección transversal (Eje X) en lugar de longitudinal. Es útil para piezas cortas con gran diámetro o para eliminar mucho material de la cara frontal.
...
N130 M30`
  ...
  {
    id: 'g04',
    title: '11. Temporización (G04)',
    module: 6,
    content: `
# Temporización G04

El comando G04 detiene el avance de los ejes durante un tiempo específico, manteniendo el cabezal girando. Es crucial para limpiar fondos de ranuras o perforaciones.

**Sintaxis:**
\`G04 X(tiempo_segundos)\` o \`G04 P(tiempo_milisegundos)\`

Ejemplo: \`G04 X1.5\` detiene el avance 1.5 segundos.
    `,
    defaultCode: `N10 G28 U0 W0
N20 T0202 (Ranurado)
N30 G97 S800 M03
...
### G97: RPM Fijas
El cabezal gira a revoluciones constantes (ej. 1000 RPM) sin importar el diámetro. Se usa en taladrado y roscado.
\`G97 S1000 M03\`

### G96: Velocidad de Corte Constante (CSS)
La máquina ajusta las RPM automáticamente según el diámetro (X) para mantener la velocidad superficial (metros/min). A menor diámetro, mayores RPM.
\`G96 S200 M03\` (200 m/min)

**Nota:** Con G96, siempre se debe limitar la velocidad máxima con **G50**.
    `,
    defaultCode: `N10 G28 U0 W0
N20 T0101
(G50 Limita RPM maximas a 2000)
N30 G50 S2000
(G96 Activa CSS a 150m/min)
N40 G96 S150 M03
N50 G00 X80 Z2
(Al bajar a X20, las RPM subiran)
N60 G01 X20 F0.2
N70 G01 Z-10
(G97 Vuelve a RPM fijas para retirarse)
N80 G97 S500
N90 G28 U0 W0
N100 M30`
  },
...
Antes del ciclo G76, las roscas se hacían pasada a pasada con G32 (o G33). G32 sincroniza el avance con el giro del husillo para crear la hélice de la rosca.

Requiere programar cada pasada de profundidad manualmente. Es útil para roscas especiales o cónicas complejas.

**Sintaxis:** \`G32 Z(final) F(paso)\`
    `,
    defaultCode: `N10 G28 U0 W0
N20 T0303 (Roscado)
N30 G97 S500 M03
...
El tronzado es la operación final donde se separa la pieza mecanizada de la barra. Se utiliza una herramienta de ranurado (lama) y se corta hasta pasar el centro (X-1 o X-2).

Es crítico reducir las RPM antes de llegar al centro (o usar G97) para evitar vibraciones o que la pieza salga disparada con fuerza excesiva.
    `,
    defaultCode: `N10 G28 U0 W0
...
Combina lo aprendido para fabricar un perno con cabeza hexagonal (simulada cilíndrica), un cuello ranurado y una rosca M20x1.5.

**Operaciones:**
1. Desbaste del perfil (G71).
2. Acabado (G70).
...
N70 G00 X19.8 (Diametro previo rosca)
